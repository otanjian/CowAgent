...
- **WHEN** 某租户管理员是其租户的有效成员且持有 `tool.execute`，目标工具为平台已开放给该租户的内置工具或租户自有 MCP 工具，而其角色未携带该工具的资源 grant
