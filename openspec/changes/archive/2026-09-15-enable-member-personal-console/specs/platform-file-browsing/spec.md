...
平台入口资格 SHALL 只允许读取其中本来就获准的平台文件，MUST NOT 绕过子路径实际归属。根内成员私有配置、记忆、会话、知识和凭证仍按本人边界及凭据规则保护；非 owner 的平台管理员不得经目录枚举、搜索、绝对路径、下载或预览取得这些内容。
...
租户成员的文件服务访问 SHALL 限于当前有效租户的共享根与获授权 Agent workspace；缺少有效租户上下文返回 403，其他租户对象按不可见拒绝。系统 MUST NOT 因客户端自报路径、租户头、Agent 标识或目录包含关系扩大范围，MUST NOT 沿用单租户 instance-root 的全局可见语义。
...
能力令牌预览（`GET /preview/<token>/<name>`）SHALL 以不可伪造的签名令牌为授权真值；其纵深根校验的允许根 SHALL 覆盖全部租户共享根与全部绑定 Agent 的 workspace（来源为服务端静态登记，不依赖请求身份），以便解析不到请求身份的公开预览仍能读取合法工作区文件；这些工作区之外的路径（含操作员主目录）MUST 仍被拒绝。该根校验 MUST NOT 通过构造「身份缺失」错误来实现，以免残留的状态或响应头污染成功响应。私有资源的预览与下载 SHALL 在**消费时**验证当前有效 owner；旧链接不能充当绕过新本人边界的持有者授权，不满足新条件的旧私有令牌必须失效。
