...
在 database 模式下，系统 SHALL 按配置与 `channel_instances` 解析并启动已启用的外部 IM 通道（与 legacy 同等的通道解析规则），MUST NOT 在启动阶段将通道列表强制过滤为仅 `web`。单通道启动失败 SHALL 记录错误且不得阻止 Web 控制台启动。

#### Scenario: 配置含飞书实例时启动
- **WHEN** database 部署的配置或 team 实例包含已启用的飞书（或其他 IM）通道
- **THEN** 进程尝试启动该通道；失败时记录错误，Web 控制台仍可用

#### Scenario: legacy 启动行为不变
- **WHEN** `identity_mode=legacy`
- **THEN** 通道启动与 Bridge 初始化行为与本 change 前一致
...
若 Desktop 企业登录或其他明确未适配客户端仍无租户边界验收，系统 SHALL 继续对其保持关闭或明确拒绝，MUST NOT 因本 change 开放 Web/通道运行而暗示 Desktop 企业入口已可用。

#### Scenario: Desktop 旧认证连接 database
- **WHEN** Desktop 使用共享密码或旧 token 连接 database 服务的企业入口
- **THEN** 系统拒绝或明确提示未开放，不回退 legacy 赋权
