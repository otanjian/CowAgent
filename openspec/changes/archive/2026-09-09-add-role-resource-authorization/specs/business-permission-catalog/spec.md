...
member与tenant_admin的原业务默认集合 SHALL 保持显式七项与九项；新增普通功能/执行权限必须明确分配，不因目录扩充自动获得。保留既有读取的兼容角色及具体资源清单 SHALL 经过迁移预检和显式应用，不能把旧关闭执行当作授权。平台管理员另按系统all获得已登记功能/资源，不受普通角色默认枚举限制；未知资源/动作及未开放消费者仍拒绝。
...
平台管理员 SHALL 拥有已登记功能和资源的系统all，并通过明确目标租户的专用平台接口管理角色授权和资源分配，目标独立验证且记录审计。租户接口的身份管理仍由当前有效tenant_admin控制，普通角色并集、部门或岗位不得产生平台/租户管理资格。平台all MUST NOT 自动建立Membership或代用目标成员执行业务；每个启用账号仍具备有效租户归属，正常业务保持租户、owner及执行约束。未适配消费者不因任何管理员资格开放。
...
功能目录、角色资源授权、模型默认值和现有分发守卫 SHALL 复用现有组件交付。目录/配置与运行状态分别标示，不以执行关闭隐藏合法目录，也不以管理完成开放执行。本change SHALL 保持正式tenant-resource-isolation的关闭边界，不接通Web消息、流/轮询/取消或其他客户端；后续开放单独验证身份、凭据、审计及适用审批/配额/隔离。平台all、导航或组件测试 MUST NOT 解锁消费者，database失败不回退legacy。
