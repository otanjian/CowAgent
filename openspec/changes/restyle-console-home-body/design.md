...
8. **纵向居中用「顶部偏移」而不是 flex 居中**。容器高 `100%` 时内容与视口等高，没有任何富余空间，`justify-content: center` 或自动外边距都无从分配，只有加高内容（顶距）才能把输入组推到中线，代价是首页由「恰好铺满」变为「可滚动」。也因为这个偏移对输入组是一段刚性平移，`--home-hero-h` 的实测与结果不构成回环，可以安全地在同一帧内反复测量。
9. **居中偏移不写死，从实测高度算**。偏移同时取决于顶栏与输入组高度，而输入组高度会被品牌说明行、窄屏字号梯度与文案换行改变，写死常量会在这些情况下失准。故 `--home-hero-h` 由 `syncHomeHeroOffset()` 实测写入，CSS 侧只留 402px 作为脚本就绪前的兜底。仓库已有 `dvh` 使用（`console.css`、`appearance.css` 的 `max-height`），因此偏移用 `dvh` 而非 `vh`。
10. **居中后折叠菜单必须收敛高度**。输入组居中后其上下余量相等（约 `(视口 - 顶栏 - 输入组) / 2`），而既有菜单上限为 320px，1280×720 下余量仅约 124px。实测 660×994 下指令菜单底部 1050 越出视口 994，且 `#chat-main` 的 `scrollHeight` 不增长——`overflow: hidden auto` 把它裁掉而非留作可滚动内容，末项不可达。故给首页菜单加 `max-height`，取输入组下方实际空间并在内部滚动；下限 96px 只在窗口矮到无法满足精确值时兜底（140px 曾在 1280×720 造成 15px 溢出）。
11. **测量的触发点必须与渲染解耦**。实测发现两个静默失效：启动时控制台仍在认证遮罩的 `hidden` 之后，`getBoundingClientRect()` 全为 0，测量被当作无效跳过；而 `requestAnimationFrame` 与 `ResizeObserver` 在被遮挡的 webview 中不回调，于是「刷新后偏移不生效、直接调用却正常」。因此 `resize` 改为同步测量，并以 `MutationObserver` 监听 `#app` 的 class 作为「认证闸门抬起」的信号——mutation 回调走微任务队列，不依赖渲染。这条同样说明：把正确性寄托在 rAF 或渲染相关观察器上，在嵌入式 webview 里是不可靠的。
...
- 纵向居中的取舍已由产品确认：红框严格居可视区正中，快捷入口排到其下方并允许首屏看不到全部卡片。若后续要求首屏同时容纳输入组与两行卡片，需改为整块居中或收回居中，届时应作为新的取舍重新记录。
- `--home-hero-h` 依赖运行时实测，CSS 只保留 402px 兜底；若将来移除该脚本或改为纯 CSS，需重新评估窄屏与品牌说明行下的居中精度。
