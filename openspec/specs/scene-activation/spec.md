...
系统 SHALL 提供 `GET /api/scenes` 返回场景分类与场景列表。接口 SHALL 在请求作用域内执行：database 身份模式下 SHALL 要求已选择有效租户（解析失败按门禁既有顺序拒绝），并 SHALL 要求调用者持有 `chat.use`（场景目录用于为会话选择场景，属 chat 消费者面；平台/租户管理员通过）。`scenes_config.json` 缺失或解析失败 SHALL 返回空结构（`scenes: []`, `categories: []`）而非 500。legacy 模式沿用既有 `_require_auth()`，行为不变。
