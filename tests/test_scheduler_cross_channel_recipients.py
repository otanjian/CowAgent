...
class _RecordingAccess:
    """Records what the tool hands to ``TaskAccessService.create_task``.

    These cases are about *which fields the tool puts in the action*, so they
    only need a sink. The real service and its tenant-member gate are exercised
    by the fork's own scheduler tests against a tenant fixture.
    """

    def __init__(self, store):
        self._store = store

    def create_task(self, actor, agent_id, task_data):
        task = dict(task_data)
