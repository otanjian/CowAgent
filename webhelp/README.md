...
> **完全离线自包含**：站点不引用任何外部网络资源。首页界面截图、部署脚本与全部能力文档的配图均已下载/转存到 `assets/` 本地，页面中不存在任何指向外网站的链接。
...
| 页面 | 说明 |
| --- | --- |
| `index.php` | 首页：Hero + 控制台界面截图、九项核心能力、企业级管控概览、模型与通道、CTA（部署命令已移至 `quickstart.php`，首页仅保留 Hero 与底部 CTA 两处入口按钮） |
...
| `manual.php` | 产品使用手册（截图为主的应用操作手册）：按「对话与协作 / 我的资源」分为两组共 5 个主题——开始对话（登录 / 新建对话 / 选智能体 / 选模型 / 发送与查看结果）、多智能体协同、设置自己的知识库（打开 / 建分类 / 建或导入文档 / 在对话里用上）、创建与配置自己的智能体、消息渠道接入；每个主题一句话定位，每个主题若干编号步骤，每步配一张当前版本真实界面截图；原理细节深链既有能力文档 |
...
`manual.php` 是面向使用者的**截图为主的应用操作手册**——讲「怎么用」，不讲「怎么装」（安装部署见 `quickstart.php`）；服务对象是控制台的成员，只收成员看得到、做得了的操作，共 5 个主题：**开始对话 / 多智能体协同 / 设置自己的知识库 / 创建与配置自己的智能体 / 消息渠道接入**。

每个主题由三部分组成：**一句话定位**（这个主题解决什么）、**若干编号步骤**（每步一句标题 + 一句操作要点，写界面上稳定的名称）、**一张当前版本的真实界面截图**（整张图是链接，点击在新标签打开原图）。需要原理、参数与边界细节时，深链既有的本地能力文档，不复制其正文。

主题分为两组，对应成员的心理模型：**对话与协作**（`conversation`：在对话界面内就能完成的事）、**我的资源**（`resources`：属于我、由我来维护的东西）。

主题顺序即阅读顺序：**先讲怎么把任务交出去，再讲怎么把自己的资源配起来**。手册正文不出现网址——需要指引时写界面路径（如「工作台 →『知识库』」「消息渠道 →『接入通道』」）或链接站内页面，不写主机名、端口与 URL。分组名（「对话与协作」「我的资源」）是手册自己的信息架构，**不是界面菜单**；正文里的界面路径一律用界面上真实存在的菜单名与按钮名（如「对话」「智能体」「知识库」「消息渠道」「接入通道」）。

结构分三层，都在 `includes/content.php`：

- `manual_parts`：手册分组（顺序即侧栏分组顺序）：对话与协作 / 我的资源。
- `manual_topics`：主题（id、`part` 分组、图标、`steps` 步骤清单、引用的文档 slug 与站内页面 id）。**数组顺序即页面与侧栏顺序**；语言包中按 id 索引，与顺序无关。
- `manual_page_links`：主题可引用的站内页面（id → 文件）。

主题由若干**编号步骤**组成：`manual_topics.<主题>.steps = [['id' => 'login', 'shot' => 'assets/img/manual/chat-1-login.jpg'], …]`。`id` 决定文案键与页内锚点（`#manual-<主题>-<步骤>`），`shot` 是这一步的截图（`assets/` 下的相对路径，走 `asset()`，因此仍然不产生任何站外请求）。步骤文案在 `lang/*.php` 的 `manual.topics.<主题>.steps.<步骤>.{title,body}`；主题本身的定位在 `manual.topics.<主题>.{nav,title,lead}`。

**截图约定**：

- 文件名 `<主题>-<序号>-<语义>.<png|jpg>`，前缀必须等于主题 id；序号即步骤顺序，中间插入步骤时要跟着重排（`tools/check-manual.php` 会断言前缀与「无重复引用、无孤儿文件」）。
- 截的是**当前版本的真实界面**，用测试账号与假数据；不做标注、拼接或只截一小块重点。
- 宽界面（对话、知识库列表）截到 1600px 宽即可，细节控件靠点击看原图。

渲染助手在 `includes/bootstrap.php`：`manual_nav()`（按分组渲染侧栏导航）、`manual_step()`（单个编号步骤：序号 + 标题 + 操作要点 + 可点开的截图）、`manual_refs()`（主题末尾的文档与页面深链）。

手册不复制既有文档正文：主题末尾的「相关文档 / 相关页面」由 slug 经 `doc_exists()` 过滤后渲染，因此文档增删不会产生死链。入口为首页 Hero 主按钮（文案键 `cta.manual`，与其它页面的 `cta.primary` 相互独立）。

**改手册内容**：在 `content.php` 增删主题或步骤、把新截图放进 `assets/img/manual/`，再到 `lang/zh.php` 与 `lang/en.php` 补齐同名 id 的文案即可，无需改 HTML。改完跑一次 `php tools/check-manual.php`。

`tools/check-manual.php` 会拦下这些情况：缺主题、缺步骤、缺标题或操作要点、截图不存在或是空文件、同一张截图被两个步骤引用、文件名前缀与主题不符、目录里存在没被引用的孤儿截图、两张截图内容完全相同（多半是抓错屏）、双语键结构不一致（某个语言包漏译）、引用了未登记的文档 slug 或未映射的页面 id。
...
│   └── check-manual.php # 手册结构、双语文案与截图一致性校验（改手册后运行）
├── includes/
...
└── assets/
    ...
    ├── img/manual/     # 产品使用手册的界面截图（<主题>-<序号>-<语义>.<png|jpg>）
    ├── video/          # 遗留演示视频（首页已改用界面截图，文件暂留）
...
- `assets/img/home-demo-console.jpg`：首页 Hero 的控制台对话界面截图（本地素材，中英文共用）。
- `assets/img/manual/`：产品使用手册的 17 张界面截图（5 个主题的编号步骤配图），按 `<主题>-<序号>-<语义>` 命名，宽度统一到 1600px。
- `assets/video/cow-demo-zh-v1.mp4`：早期演示视频（约 37MB）。首页 Hero 已改用界面截图，该文件不再被页面引用，暂留作素材备份。
...
- **改产品使用手册**：分组在 `includes/content.php` 的 `manual_parts`，主题在 `manual_topics`（id、`part`、图标、`steps` 步骤清单、`docs` / `links` 深链，**数组顺序即页面顺序**）；步骤的 `shot` 指向 `assets/img/manual/` 下的截图。主题定位在 `lang/*.php` 的 `manual.topics.<主题>.{nav,title,lead}`，步骤文案在 `manual.topics.<主题>.steps.<步骤>.{title,body}`。增删主题或步骤只改 `content.php`、放好截图、补齐两个语言包，无需动 HTML。手册以**真实界面截图 + 编号步骤**承载操作，原理细节一律深链既有文档；正文不写网址（主机名、端口、URL 一律不出现），要指路就写界面路径或链站内页面。改完运行 `php tools/check-manual.php`。
