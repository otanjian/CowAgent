...
ROUTES: Tuple[RouteEntry, ...] = (
    ...
    RouteEntry("/api/logs/download", "LogsDownloadHandler", "upstream", {"GET": P("tenant", comment="logs download")}),
    RouteEntry("/api/logs", "LogsHandler", "upstream", {"GET": P("tenant", comment="logs")}),
    ...
    RouteEntry("/api/scenes", "ScenesHandler", "fork:scenes", {"GET": P("tenant", comment="scene catalog for the current tenant")}),
    RouteEntry("/api/scenes/activate", "SceneActivateHandler", "fork:scenes", {"POST": P("tenant", comment="activate a scene (state change; origin+CSRF in handler)")}),
    RouteEntry("/api/scenes/workbench/import", "SceneWorkbenchImportHandler", "fork:scenes", {"POST": P("tenant", comment="import workbench content (state change; origin+CSRF in handler)")}),
