    ...
    def handle_single(self, cmsg: DingTalkMessage):
        ...
        if context:
            ...
            stamp_external_identity(
                ...
                # ``sender_staff_id`` only exists on the DingTalk message
                # subclass; a bare ``ChatMessage`` (as tests and other
                # callers build) carries just the resolved author id.
                subject=cmsg.actual_user_id or getattr(cmsg, "sender_staff_id", "") or "",
            )
