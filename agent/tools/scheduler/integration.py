...
def init_scheduler(agent_bridge, workspace_root: str = None, agent_id: str = None) -> bool:
    ...
    with _init_lock:
        ...
        try:
            ...
            # The store above is the one *global* store, and one service now
            # runs every Agent's tasks, so the Agent is resolved per task
            # (upstream's `_resolve_task_agent_id`) instead of being fixed in a
            # closure. The fork's callback still owns the owner-revalidation
            # policy and the identity resolver; upstream's `_run_scheduled_task`
            # does the dispatch, so every attempt records a run.
            def execute_task_callback(task: dict, trigger: str = "scheduled"):
                agent_id = _resolve_task_agent_id(agent_bridge, task)
                return _make_execute_callback(agent_bridge, agent_id, task_store)(
...
def _make_execute_callback(agent_bridge, agent_id: str, task_store):
    ...
    def execute_task_callback(task: dict, trigger: str = "scheduled"):
        ...
        try:
            ...
            # no fork-only identity branch to conflict on. Dispatch is
            # upstream's `_run_scheduled_task`, which opens a run row around
            # the attempt and threads the run's `trigger` through.
...
class AgentScopedTaskStore:
    """One Agent's view over the global ``TaskStore``.

    Tasks live in a single file now, with the Agent they run as stamped on each
    task. Callers that think per Agent -- the authorization service resolves
    "the store for agent X", and the console lists one Agent's schedule -- keep
    that call shape through this view, while every read and write still lands in
    the one file the scheduler loop reads:

    * ``list_tasks`` is filtered with the view's ``agent_id`` (the store's own
      bucketing, which follows a task's *effective* owner);
    * ``add_task`` stamps the Agent, so a task created through the view is never
      filed under the default Agent by omission;
    * everything else delegates, because task ids are unique across the store.
    """
    ...
    def list_tasks(self, enabled_only: bool = False, agent_id: str = None):
        # The view's Agent wins: a caller holding a scoped store must not be
        # able to widen it by passing a different id.
        return self._store.list_tasks(
            enabled_only=enabled_only, agent_id=self._agent_id
        )

    def add_task(self, task: dict) -> bool:
        if isinstance(task, dict):
            task.setdefault("agent_id", self._agent_id)
...
def get_scoped_task_store(agent_id: str):
    """``get_task_store()`` filtered to one Agent (``AgentScopedTaskStore``)."""
