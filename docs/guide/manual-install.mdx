...
description: Deploy 容大AI manually (source code / Docker)
---

<Note>
  The public installers, repository URLs, and container images referenced here are provided by the upstream project. For **容大AI** with this repository’s branding and changes, use this repository’s source and local build artifacts.
</Note>
...
<Tip>
  **Deploying on a server?** By default `web_host` only listens on `127.0.0.1` (local access). Set `web_host` to `0.0.0.0` in `config.json` to make the console reachable from outside, then sign in with a database account (username + password). On first start the platform admin is bootstrapped with a one-time password. Don't forget to open port `9899` in your firewall or security group — ideally restricted to specific IPs.
</Tip>
...
<Tip>
  **Running in Docker?** Set `WEB_HOST` to `0.0.0.0` in `docker-compose.yml` so the console is reachable from outside the container, then sign in with a database account (username + password). First start bootstraps the platform admin with a one-time password. Make sure port `9899` is mapped to the host and open in your firewall or security group.
</Tip>
