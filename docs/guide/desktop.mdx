---
title: Upstream Desktop Client
description: Download and use the upstream desktop client (macOS / Windows)
---

<Note>
  This page describes the **upstream project** desktop distribution. These public downloads do not contain this repository’s 容大AI branding and changes.
</Note>

The upstream project ships a ready-to-use desktop client with the Agent runtime bundled in — **no need to install Python or dependencies manually**. Just download, install, and run your local super AI assistant.

<Frame caption="It drives your browser, terminal, and file system directly, with long-term memory, a personal knowledge base, and self-evolution — all running locally.">
  <video controls preload="metadata" playsInline className="w-full rounded-xl" src="https://www.rsm.global/china/zh-hans" />
</Frame>

## Download & Install

<Card title="Download the upstream client" icon="download" href="https://www.rsm.global/china/zh-hans">
  Download the macOS / Windows installer
</Card>

<Tabs>
  <Tab title="macOS">
    1. Open the [upstream project download page](https://www.rsm.global/china/zh-hans) and pick the build for your chip:
       - Apple Silicon (M1/M2/M3/M4): download the `arm64` build
       - Intel: download the `x64` build
    2. Open the downloaded `.dmg` and drag the desktop client into your Applications folder.
    3. Launch the desktop client from Launchpad or Applications.
  </Tab>
  <Tab title="Windows">
    1. Open the [upstream project download page](https://www.rsm.global/china/zh-hans) and download the Windows installer (`.exe`).
    2. Run the installer and follow the prompts (you can choose the install directory).
    3. Launch the desktop client from the desktop or Start menu.
  </Tab>
</Tabs>
