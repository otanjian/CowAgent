...
description: Manage 容大AI process lifecycle with cow commands
---

Process management commands control the 容大AI background process. These commands are only available in the terminal.

## start

Start the 容大AI service. Runs as a background daemon by default and automatically tails logs.
...
Restart the 容大AI service (stop then start).
...
Check 容大AI service status, including process info, version, and current model/channel configuration.
