---
title: v2.1.7
description: "RongAI 2.1.7: multiple workspaces isolated per session, session-level permission modes, task notifications and desktop improvements, and new models"
---
...
Docs: [Workspace](https://www.rsm.global/china/zh-hans)
...
> Later change: in multi-tenant (database) installations execution is now decided by the caller's **role resource authorization**, so the in-conversation permission switch was removed and the global default is read-only there. Single-tenant installs keep the global default. See [Architecture](/intro/architecture#session-settings).
...
- **Desktop client**: check for updates and update with one click inside the client, or get the latest version from the [download page](https://www.rsm.global/china/zh-hans).
