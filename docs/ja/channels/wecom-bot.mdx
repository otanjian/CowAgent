---
title: WeCom Bot
description: 容大AI を WeCom AI Bot に接続する（WebSocket ロングコネクション）
---

WeCom AI Bot を介して 容大AI を接続し、ダイレクトメッセージとグループチャットの両方に対応します。パブリック IP は不要で、WebSocket ロングコネクションを使用し、Markdown レンダリングとストリーミング出力をサポートします。
