---
title: Telegram
description: Telegram Bot API 経由で 容大AI を接続
---

> 公式の Telegram Bot API を通じて 容大AI を接続します。1 対 1 チャットおよびグループチャット（@メンションまたはボットへの返信で起動）に対応。Long Polling 方式のため公開 IP は不要で、すぐに利用できます。
