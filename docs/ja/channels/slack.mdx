---
title: Slack
description: Slack App 経由で 容大AI を接続
---

> Slack App の **Socket Mode** を通じて 容大AI を接続します。ダイレクトメッセージ（DM）およびチャンネル（@メンションまたはスレッド内の返信で起動）に対応。Socket Mode は WebSocket の常時接続を使うため公開 IP やコールバック URL は不要で、すぐに利用できます。
...
2. **App Name**（例: `容大AI`）を入力し、インストール先の **Workspace** を選択して作成します。
