...
<CardGroup cols={2}>
  ...
  <Card title="search_files - ファイル検索" icon="magnifying-glass" href="/ja/tools/search-files">
    ファイル内容の検索、またはファイル名でファイルを探す
  </Card>
  ...
  <Card title="agent_delegate - 委譲" icon="users" href="/ja/tools/delegate">
    セッション内の別の Agent にタスクを渡す
  </Card>
  <Card title="subagent - サブ Agent" icon="users-gear" href="/ja/tools/subagent">
    一時的なサブ Agent を作成して独立したタスクを並行実行する
  </Card>
</CardGroup>
