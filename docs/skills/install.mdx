...
容大AI supports installing skills from [Cow Skill Hub](https://www.rsm.global/china/zh-hans), GitHub, ClawHub, LinkAI, and any URL via a unified `install` command. Use `/skill install` in chat or `cow skill install` in the terminal.

## From the Skill Hub

Browse all available skills at [skills.cowagent.ai](https://www.rsm.global/china/zh-hans) and install by name:
