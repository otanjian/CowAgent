> 2026-09-17 最新实施：已按新的原样迁移要求，将 OneAgent 场景应用归入 `Scene/<scene.id>/`。配置、专用工作台、技能与后端均已迁入；详见 [Scene/README.md](../../Scene/README.md)。下文保留之前清空目录和早期简化移植的历史记录。

# 场景应用回流实现方案

> 2026-09-17 更新：原内置的 10 个分类、26 个场景及 81 个子场景已清空，13 个业务技能包和 6 个前端专用工作台已删除。当前仅保留场景应用入口、空目录和通用基础能力。以下为历史实现记录，不代表当前随仓库分发的内容。
