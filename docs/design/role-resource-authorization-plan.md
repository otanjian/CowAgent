...
| 平台管理员 | `User.is_platform_admin` 资格及明确的平台管理接口 | 对全部已登记权限和上述资源动态生效的统一 all 判断 |
...
沿用 `User.is_platform_admin` 作为唯一平台资格来源，由服务端派生 `authorization_mode=all`。不把平台管理员伪装成每个租户都存在的一条普通角色记录，也不允许角色保存接口写入 `*` 或该身份标识。
