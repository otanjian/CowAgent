---
title: WeChat
description: Connect 容大AI to personal WeChat (via the official API)
---

> Connect 容大AI to your personal WeChat — scan to log in, no public IP required. Supports text, image, voice, file, and video messages in 1-on-1 chats. Backed by WeChat's official API; safe to use. After connecting, a bot assistant is added to your conversation list without affecting normal account usage.

## 1. Setup and run

### Option A: Web console

Start 容大AI and open the Web console (local URL: http://127.0.0.1:9899/). Go to the **Channels** tab, click **Connect Channel**, select **WeChat**, and follow the prompts to scan in.
...
After starting 容大AI, a QR code is displayed in the terminal. Scan it with WeChat to complete login.
...
When the WeChat session expires (errcode `-14`), 容大AI automatically clears old credentials and initiates a new QR login — no manual intervention required.
