---
title: 手动安装
description: 手动部署 容大AI（源码 / Docker）
---
...
<Tip>
  **服务器公网访问 Web 控制台**：默认 `web_host` 仅监听 `127.0.0.1`（本机访问），需公网访问时请在 `config.json` 中将 `web_host` 设为 `0.0.0.0`，然后使用数据库账号（用户名 + 密码）登录；首次启动会自动初始化平台管理员并输出一次性密码。此外还需在防火墙/安全组中放行 `9899` 端口，建议仅对指定 IP 开放以保证安全。
</Tip>
...
<Note>
  **数据持久化**：`docker-compose.yml` 默认将两个目录挂载到宿主机，容器重启、`docker compose pull` 升级换镜像后数据都不会丢失：

  - `./cow` → 容器内 `/home/agent/cow`：工作空间，存放对话安装的技能、记忆、知识库、`mcp.json` 及对话产物。
  - `./cow-data` → 容器内 `/home/agent/.cow`：数据目录，由 `COW_DATA_DIR` 指定，存放 Web 控制台保存的 `config.json`、运行日志、通道凭据等。

  两个目录默认位于 `docker-compose.yml` 同级路径下，请勿删除。
</Note>

<Tip>
  **Docker 公网访问 Web 控制台**：在 `docker-compose.yml` 中将 `WEB_HOST` 设为 `0.0.0.0`（容器内默认绑定 `127.0.0.1` 无法从宿主机外访问），然后使用数据库账号（用户名 + 密码）登录；首次启动会自动初始化平台管理员并输出一次性密码。此外需确保 `9899` 端口正确映射到宿主机，并在防火墙/安全组放行该端口。
</Tip>
...
| `model` | `MODEL` | 模型名称 | `deepseek-flash` |
