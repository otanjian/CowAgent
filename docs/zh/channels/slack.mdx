---
title: Slack
description: 将 容大AI 接入 Slack App
---

> 通过 Slack App 的 **Socket Mode** 接入 容大AI，支持私聊（DM）与频道（@机器人 / 线程内回复触发）。Socket Mode 基于长连接，无需公网 IP 与回调地址，开箱即用。
...
2. 填写 **App Name**（如 `容大AI`），选择要安装的 **Workspace**，点击创建。
