...
| 参数 | 说明 |
| --- | --- |
| `model` | 支持 `deepseek-flash`（默认，V4.1 Flash，原生多模态）、`deepseek-v4-flash`、`deepseek-v4-pro` |
...
| 模型 | 适用场景 |
| --- | --- |
| `deepseek-flash` | 默认推荐，V4.1 Flash，原生多模态、速度快、成本低 |
...
- **采样参数**：思考模式下 `temperature`、`top_p`、`presence_penalty`、`frequency_penalty` 会被服务端忽略（不会报错），容大AI 会自动跳过传入。
- **多轮工具调用**：当历史中包含工具调用时，DeepSeek 要求所有 assistant 消息必须回传 `reasoning_content`。容大AI 会自动处理回传逻辑，跨轮次切换思考开关也不会出错。

<Tip>
  默认使用 `deepseek-flash`（V4.1 Flash，原生多模态）；复杂任务可使用 `deepseek-v4-pro`；需要深度推理可开启 `enable_thinking`。
</Tip>
