---
title: Architecture
description: 容大AI 2.0 system architecture and core design
---

容大AI is an out-of-the-box super AI assistant and a complete Agent harness framework, featuring complex task planning, long-term memory, skill extensibility, self-evolution, and multi-agent collaboration.

## System Architecture

容大AI's architecture consists of the following core modules:

<img src="https://cdn.jsdelivr.net/gh/zhayujie/cowagent-assets@main/architecture/en/architecture.jpg" alt="容大AI Architecture" />
...
When Agent mode is enabled, 容大AI runs as an autonomous agent with the following workflow:
...
Workspace and model can be **set per session**, falling back to the global default when unset:

- **Model**: different sessions can switch to different models, making it easy to pick the right one per task.
- **Permission**: controls what the Agent is allowed to do, in three levels — **read-only**, **workspace-write**, and **full-access**. On a single-tenant install you set a global default (`agent_permission_mode`) in the config and new sessions inherit it. On a multi-tenant (database) install the setting is read-only: what a session may actually run is decided by the caller's **role resource authorization** (`tool.execute` plus the resource grant), which an administrator assigns — the conversation itself offers no way to widen it. Permissions reduce the risk of accidental changes; for strong isolation, run inside a container.
